local EVENT_NAMESPACE = 'sfgdndhjjfdgkjhfk'
...
function addon:Initialize(sv)
	...
	local perfMeter = CreateControlFromVirtual('$(parent)PerformanceMeter', control, 'IMP_KillLocations_PerformanceMeterTemplate')
...
function addon:UpdateMap()
	if WORLD_MAP_SCENE:IsHiding() then
		self.mapIsDirty = true
		-- df('Map is hidden, don\'t update map')
		return
	end

	if not self.mapIsDirty then
		-- df('Map isn\'t  dirty, don\'t update map')
		return
	end

	-- df('Map going to be updated')
